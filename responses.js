...
//hydration reminders
var reminder = [
    "Don't forget to hydrate today, got it?", 
    "Remember to drink water today.", 
    "Drink some water today, alright?", 
    "Remember to stay hydrated today.", 
    "Don't forget to drink water today.", 
    "Stay hydrated, alright?", 
    "Remember to drink water. It's good for you.", 
...
var yare = [
    ...
    "Hey, that's my line.", 
    "Oi, that's my line.", 
    "You trying to steal my line?"
];

function sendGoodMorning(message) {
    var morning = ["Good morning.", "Morning."];
    message.channel.send(`${morning[functions.getRandom(morning.length)]} ${reminder[functions.getRandom(reminder.length)]}`);
}
...
for (const ty in tys) {
    if (msg.content.includes(`${tys[ty]} jotaro`) || msg.content.includes(`${tys[ty]} jotarobot`)) {
